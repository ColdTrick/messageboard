...
<div class="messageboard"><!-- start of messageboard div -->
    ...
    <div class="message_sender">	        
        <?php
			if($vars['annotation']->owner_guid){
				echo elgg_view("profile/icon",array('entity' => get_entity($vars['annotation']->owner_guid), 'size' => 'tiny'));
			} else {
				// handling anonymous data
				if(stristr($vars['annotation']->value, "#msga#")){
					$msgdata = split("#msga#", $vars['annotation']->value);
					$anonymous_name = $msgdata[0];
					$vars['annotation']->value = $msgdata[1];
				}
			}
        ?>
    </div>
    ...
    <p class="message_item_timestamp">
        <?php if($anonymous_name){	
				echo $anonymous_name;
			} else {
				echo get_entity($vars['annotation']->owner_guid)->name;
			} 
	...
	?>
		        <?php
		            ...
		            if($vars['annotation']->owner_guid != $_SESSION['guid'] && $vars['annotation']->owner_guid){
